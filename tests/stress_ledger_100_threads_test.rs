...
fn test_stress_ledger_100_concurrent_threads_20k_txs() {
    ...
    for t_id in 0..num_threads {
        ...
        let handle = thread::spawn(move || {
            for i in 0..txs_per_thread {
                ...
                let res = acc.record_transaction(
                    amount,
                    entry_type,
                    Some(format!("TX-100T-{}-{}", t_id, i)),
                );
    ...
    assert!(
        account.verify_integrity().unwrap(),
        "Integridade de SHA-256 no Ledger violada!"
    );
}
...
fn test_ledger_insufficient_funds_rejection_under_concurrency() {
    ...
    assert_eq!(
        successes, 2,
        "Apenas 2 saques poderiam ser permitidos com saldo R$ 10,00"
    );
    assert_eq!(
        failures, 8,
        "8 saques deveriam ter sido rejeitados por saldo insuficiente"
    );
    assert_eq!(
        account.get_balance_cents().unwrap(),
        0,
        "Saldo final DEVE ser exatamente 0"
    );
