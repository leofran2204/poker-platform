-- wipe-synthetics.sql — Remove contas de teste sintéticas (preserva humanas e @smoke.local admin)
-- Backup: pg_dump -U user -d poker_db > backups/pre_wipe_synthetics_$(date +%Y%m%d_%H%M%S).sql
BEGIN;
CREATE TEMP TABLE _wipe_ids ON COMMIT DROP AS
  SELECT id FROM users
  WHERE email ILIKE '%@example.com'
     OR email ILIKE '%@live-sim.test'
     OR email ILIKE '%@test.local'
     OR email ILIKE '%@%emalupe.com';
CREATE TEMP TABLE wipe_seats ON COMMIT DROP AS
  SELECT s.id AS seat_id, s.user_id, s.table_id, s.chips, s.wallet_kind
  FROM cash_game_seats s JOIN _wipe_ids w ON w.id=s.user_id WHERE s.status='ACTIVE';
UPDATE users SET balance_pm_cash=balance_pm_cash+c.pm_cash, balance=balance_pm_cash+c.pm_cash, balance_real=balance_real+c.real
FROM (SELECT user_id, COALESCE(SUM(chips) FILTER(WHERE wallet_kind='pm_cash'),0)::bigint pm_cash,
             COALESCE(SUM(chips) FILTER(WHERE wallet_kind='real'),0)::bigint real FROM wipe_seats GROUP BY user_id) c
