...
echo "${LOG_TAG} docker compose up -d"
docker compose up -d
...
if command -v curl >/dev/null 2>&1; then
  curl -fsS -o /dev/null -w "caddy-health HTTP %{http_code}\n" http://127.0.0.1/caddy-health || echo "caddy-health FAIL"
  curl -fsS -o /dev/null -w "api-via-proxy /health %{http_code}\n" http://127.0.0.1/health || true
else
  wget -qO- http://127.0.0.1/caddy-health && echo || echo "caddy-health FAIL"
fi
