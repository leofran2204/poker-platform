...
## 0. 🦀 Stack Alvo — Rust para Toda a Plataforma (atualizado em 2026-06-30)

| Camada            | Linguagem                      | Responsabilidade                                  |
|-------------------|--------------------------------|---------------------------------------------------|
| **Backend**       | **Rust**                       | Motor de jogo, APIs, autenticação, lobby          |
| **IA e Dados**    | **Python**                    | Antifraude, estatísticas, relatórios              |
| **Front-end**     | **TypeScript (React/Next.js)** | UI para jogadores e administradores               |
| **Comunicação**   | **JSON**                       | Formato universal entre módulos                   |

> 📐 Detalhes completos em `07-Arquitetura-Motor/ARQUITETURA_MOTOR.md` (versão 2.0).
> ⚠️ O MVP atual em Node.js será descontinuado quando o backend Rust estiver maduro.
...
## 12. 🔗 Referências no Código — Onde Cada Regra Vive

| Regra               | Arquivo                               | Função/Local                    |
|---------------------|---------------------------------------|---------------------------------|
| Baralho 36 cartas   | `server/src/engine/deck.ts`           | `RANKS`, `createDeck()`         |
| Flush > Full House  | `server/src/engine/deck.ts`           | `evaluateHand()` (ordem 7→6)    |
| A-6-7-8-9 straight  | `server/src/engine/deck.ts`           | `hasStraight()`                 |
| Timeouts            | `server/src/engine/table-manager.ts`  | `ACTION_TIMEOUTS`               |
| Validação de mesa   | `server/src/routes/tables.ts`          | `createTableSchema`             |
| Fluxo de jogo       | `server/src/engine/table-manager.ts`  | `startHand()`, `advanceGame()`  |
| Ações               | `server/src/engine/table-manager.ts`  | `handleAction()`                |
| Showdown            | `server/src/engine/table-manager.ts`  | `endHand()`                     |
| Tipos               | `shared/types.ts`                     | Todos os tipos                  |

---

## 13. 🏛️ Sistema de Lobby — Gestão de Mesas e Matchmaking

### 13.1 🎛️ Gestão de Mesas — Criação e Listagem
...
### 13.2 🚪 Matchmaking e Entrada — Fluxo do Jogador
- **Fluxo de Entrada**:
...
### 13.3 📡 Sincronização em Tempo Real — WebSocket
...
## 14. 🕵️ Sistema Antifraude — Collusion, Bots e Chip Dumping

### 14.1 🤝 Detecção de Conluio — Collusion e Soft Play
...
### 14.2 💸 Chip Dumping — Transferência Fraudulenta
...
### 14.3 🤖 Detecção de Bots — Automação e GTO
...
### 14.4 👥 Multi-accounting — Fingerprinting e Duplicidade
