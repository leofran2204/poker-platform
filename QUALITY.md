...
> - `07-Arquitetura-Motor/ARQUITETURA_MOTOR.md` — arquitetura técnica do motor
> - `05-Documentacao/BUSINESS_RULES.md` — 45 regras de negócio do poker
> - `05-Documentacao/STATUS.md` — spec viva e progresso dos módulos
> - `05-Documentacao/CRONOGRAMA.md` — fases e timeline
> - `05-Documentacao/DASHBOARD.md` — painel tático de tarefas
...
*   **Instrutor:** Mark — especialista em Lógica, Spec-Driven Development (SDD),
    Harness Engineering, **Rust (Linguagem Principal / Backend)**, Python (IA),
    TypeScript (Frontend), CRMs, APIs REST, MCP, MCP Server, WebMCP, UCP do Google,
...
*   **Python (futuro):** Agentes inteligentes, suporte ao cliente, análise de dados.
...
7.  **Programação Agêntica & Python** — orquestração com agentes, WebMCP, UCP.
...
## ⚙️ 2.5.1 REGRA DO TOOLCHAIN — Como Rodar Cargo no 09-Frontend-Dioxus
...
cd "c:\Users\leofr\OneDrive\Projetos\Poker_Project\09-Frontend-Dioxus"
...
- **NUNCA** rode `cargo check`/`cargo test`/`cargo clippy` diretamente no
  `09-Frontend-Dioxus/` sem configurar as variáveis — vai falhar.
- `08-Motor-Rust/` **não** precisa dessa configuração (não usa proc-macros DLLs).
...
### Frontend (09-Frontend-Dioxus)
```toml
[dependencies]
dioxus = "0.6"          # Framework frontend (React-like)
...
| **loss_deflator.rs**    | Cashback por tiers (15/25/35%)                 | Perda de R$100 → cashback correto por tier; limite máximo respeitado                                |
...
**Status:** ⏳ Pendente | **Arquivo a criar:** `05-Documentacao/BUG_REPORT_TEMPLATE.md`
...
- [ ] Poker variants (Omaha, Stud, Short Deck)
- [ ] Esports poker
- [ ] IPO ou aquisição
...
|-------------------------------|----------------------------------------------------|------------------------------|
| **Loss Deflator**             | Cashback automático por perdas (15/25/35%)         | Alta — reduz churn de perdedores |
...
| Tier de Perda                       | Cashback   | Frequência  | Limite/mês   |
|-------------------------------------|------------|-------------|--------------|
| **Tier 1** (R$ 0-500 perdidos)      | 15%        | Semanal     | R$ 75        |
| **Tier 2** (R$ 500-2.000)           | 25%        | Semanal     | R$ 500       |
| **Tier 3** (R$ 2.000+)              | 35%        | Semanal     | R$ 1.000     |

> **Diferencial:** Nenhuma outra plataforma oferece cashback automático por
> perdas. Isso reduz o churn de jogadores que perdem e desanimam.
...
| **Shallow Work**    | Tarefas administrativas, emails, mensagens, reuniões curtas                              | Responder issues, atualizar STATUS.md, revisar PRs simples — 1-2h/dia                                      |
...
- 1 Pomodoro = 1 tarefa do `STATUS.md` ou 1 bug do `DEVELOPMENT_LOG.md`
...
| **3. Organizar**    | Colocar em lugares certos                  | Projetos → STATUS.md; Ações → Kanban; Referência → docs/               |
| **4. Refletir**     | Revisar semanalmente                       | Revisão semanal: STATUS.md, Kanban, OKRs, inbox                        |
...
|---------------------------|-------------------------------------|-----------------|
| `STATUS.md`               | Progresso de features e módulos     | Semanal         |
...
| **Fim do dia**        | Atualizar STATUS.md e Kanban                            | 10 min      |
...
jobs:
  ...
  iac:
    steps:
      ...
      - run: checkov -d 04-Infraestrutura-Docker/
...
│          │                │                │                     │
│  ┌───────▼──────┐  ┌──────▼───────┐  ┌────▼────────┐            │
│  │ README.md    │  │ STATUS.md    │  │ CRONOGRAMA  │            │
│  │ (visão geral)│  │ (progresso)  │  │ .md (prazos)│            │
│  └──────────────┘  └──────────────┘  └─────────────┘            │
│                                                                 │
│  ┌──────────────┐  ┌──────────────┐  ┌─────────────┐            │
│  │ BUSINESS_    │  │ DASHBOARD.md │  │ ARQUITETURA │            │
│  │ RULES.md     │  │ (métricas)   │  │ _MOTOR.md   │            │
│  └──────────────┘  └──────────────┘  └─────────────┘            │
│                                                                 │
│  ┌──────────────┐  ┌──────────────┐  ┌─────────────┐            │
│  │ DEVELOPMENT  │  │ ESTRATEGIA_  │  │ AUDITORIA_  │            │
│  │ _LOG.md      │  │ APRENDIZADO  │  │ REGRAS.md   │            │
│  └──────────────┘  └──────────────┘  └─────────────┘            │
└─────────────────────────────────────────────────────────────────┘
```
...
| **STATUS.md**                    | Progresso atual, o que está feito, o que falta               | Início de cada sessão               |
...
| **ESTRATEGIA_APRENDIZADO.md**    | Estratégia de aprendizado (Rust, poker, segurança)           | Estudo                              |

## 🗂️ 18.3 ESTRUTURA DE PASTAS E PROPÓSITO — Poker Online

| Pasta                          | Propósito                          | Conteúdo Principal                                      |
|--------------------------------|------------------------------------|---------------------------------------------------------|
| `08-Motor-Rust/`               | Motor de poker em Rust             | `src/` (8 módulos + 4 antifraude), `tests/`             |
| `09-Frontend-Dioxus/`          | Frontend WebAssembly               | `src/main.rs`, `Dioxus.toml`                            |
| `04-Infraestrutura-Docker/`    | Infraestrutura como código         | `docker-compose.yml`, `render.yaml`                     |
| `05-Documentacao/`             | Documentação do projeto            | Todos os `.md` de documentação                          |
| `07-Arquitetura-Motor/`        | Arquitetura do motor               | `ARQUITETURA_MOTOR.md`                                  |
| `scripts/`                     | Scripts de automação               | `coverage.ps1`, `coverage.sh`                           |
