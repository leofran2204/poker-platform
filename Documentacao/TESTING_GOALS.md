...
> **Atualização 2026-08-29 (S18):** validação do catálogo cash oficial — `Motor-Rust/tests/cash_catalog_10k_hands.rs` (10k mãos × NLHE/SD/SD Omaha); `short_deck_massive`; smoke `scripts/live-e2e-seeded-catalog.mjs` (Real e Play, mesa a mesa). Stress pode usar `GameLoop::with_skip_loss_deflator(true)` para evitar MC multiway lento.
>
> **Atualização 2026-09-02 (S20f):** `Motor-Rust/tests/tournament_to_champion.rs` (MTT até 1 campeão, 1 rebuy, reservas após rebuy); `scripts/live-sim-full-ritual.mjs` (Play Money, 1 e-mail/assento). 2.000 mãos **ao vivo no WS** não são o gate do motor (10k já cobre); o WS mede rede + timeout 30 s + pausa 6 s entre mãos.
>
> **Gate 2026-08-31 (S19):** `cargo fmt --check`, Clippy `--all-targets -D warnings`, 51 testes ativos selecionados da API, 4 contratos financeiros PostgreSQL isolados, TypeScript e build Vite — zero falhas. O contrato DePix comprova crédito único em `completed`, deduplicação e ausência de crédito em `processing`.
