...
fn main() {
    ...
    println!("   - Latência Médias: {:.2} ns ({:.3} µs) por avaliação", nanos_per_eval, nanos_per_eval / 1000.0);
    println!("   - Throughput: {:.2} avaliações/segundo\n", (iterations as f64) / elapsed.as_secs_f64());
    ...
    let contributions = vec![
        ...
        Contribution { player_id: "P4".into(), total_bet: 1000.0, has_folded: false },
    ];
    ...
    println!("   - Latência Média: {:.2} ns ({:.3} µs) por cálculo", nanos_per_side_pot, nanos_per_side_pot / 1000.0);
    println!("   - Throughput: {:.2} cálculos/segundo\n", (iterations as f64) / elapsed.as_secs_f64());
    ...
    println!("   - Latência Média: {:.2} ns ({:.3} µs) por transação", nanos_per_tx, nanos_per_tx / 1000.0);
    println!("   - Throughput: {:.2} transações/segundo\n", 10_000.0 / elapsed.as_secs_f64());
    ...
    println!("   - Latência Média: {:.2} ns ({:.3} µs) por embaralhamento", nanos_per_shuffle, nanos_per_shuffle / 1000.0);
    println!("   - Throughput: {:.2} embaralhamentos/segundo\n", 10_000.0 / elapsed.as_secs_f64());

    // 5. Benchmark: Balanceador Dinâmico de Mesas de Torneio
    let tables = vec![
        TableStateSummary { table_id: "T1".into(), active_player_ids: vec!["P1".into(), "P2".into(), "P3".into(), "P4".into(), "P5".into(), "P6".into()] },
        TableStateSummary { table_id: "T2".into(), active_player_ids: vec!["P7".into(), "P8".into(), "P9".into()] },
    ];
    let start = Instant::now();
    for _ in 0..iterations {
    ...
    println!("   - Latência Média: {:.2} ns ({:.3} µs) por análise", nanos_per_balance, nanos_per_balance / 1000.0);
    println!("   - Throughput: {:.2} análises/segundo", (iterations as f64) / elapsed.as_secs_f64());
