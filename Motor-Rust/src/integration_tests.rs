...
use crate::utils::{pots_elegeiveis, soma_total_pots};
...
fn test_side_pots_com_rake_completo() {
    ...
    let _community = [c(Rank::Two, Suit::Clubs),
...
fn test_fluxo_completo_cash_game() {
    ...
    // Avalia mãos
    let _hand1 = evaluate_hand(&hole1, &community_cards);
    let _hand2 = evaluate_hand(&hole2, &community_cards);
