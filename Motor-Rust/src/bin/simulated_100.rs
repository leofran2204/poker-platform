use poker_engine::deck::evaluate_hand_short_deck;
use poker_engine::game_loop::{GameLoop, PlayerMove};
use poker_engine::hand_history::GameType;
use poker_engine::tournament_engine::{
    create_tournament, eliminate_player, register_player, start_tournament, BlindLevel,
...
fn main() {
    ...
    for (variant, max_players, sb, bb, _buyin, label) in configs {
        let cfg = TableConfig::new(bb, 500, 500)
            .with_small_blind(sb)
        ...
        // verifica ranking Short Deck: trips > straight e flush > FH via evaluate
        {
            ...
            let trips = evaluate_hand_short_deck(
                &[
                    poker_engine::deck::Card {
                        rank: poker_engine::deck::Rank::Seven,
                        suit: poker_engine::deck::Suit::Hearts,
                    },
                    poker_engine::deck::Card {
                        rank: poker_engine::deck::Rank::Seven,
                        suit: poker_engine::deck::Suit::Diamonds,
                    },
                ],
                &[
                    poker_engine::deck::Card {
                        rank: poker_engine::deck::Rank::Seven,
                        suit: poker_engine::deck::Suit::Clubs,
                    },
                    poker_engine::deck::Card {
                        rank: poker_engine::deck::Rank::King,
                        suit: poker_engine::deck::Suit::Spades,
                    },
                    poker_engine::deck::Card {
                        rank: poker_engine::deck::Rank::Nine,
                        suit: poker_engine::deck::Suit::Hearts,
                    },
                    poker_engine::deck::Card {
                        rank: poker_engine::deck::Rank::Eight,
                        suit: poker_engine::deck::Suit::Clubs,
                    },
                    poker_engine::deck::Card {
                        rank: poker_engine::deck::Rank::Six,
                        suit: poker_engine::deck::Suit::Hearts,
                    },
                ],
            );
            let straight = evaluate_hand_short_deck(
                &[
                    poker_engine::deck::Card {
                        rank: poker_engine::deck::Rank::Ten,
                        suit: poker_engine::deck::Suit::Hearts,
                    },
                    poker_engine::deck::Card {
                        rank: poker_engine::deck::Rank::Six,
            ...
            let flush = evaluate_hand_short_deck(
                ...
                &[
                    ...
                    poker_engine::deck::Card {
                        rank: poker_engine::deck::Rank::Jack,
                        suit: poker_engine::deck::Suit::Diamonds,
                    },
                ],
            );
        ...
        for hand_idx in 0..100_000 {
            ...
            if res.rake > 0 && hand_idx % 37 == 0 {
                // ~2.7% das mãos simulam trigger
                deflator_triggers += 1;
        ...
        println!(
            "  {}: 100k mãos, rake {} cents, deflator checks flush>FH {} trips>straight {}",
            label, total_rake, flush_beats_fh, trips_beats_straight
        );
    }
    ...
    let blind_levels: Vec<BlindLevel> = (0..26)
        .map(|i| BlindLevel {
            level: (i + 1) as u32,
            small_blind: 25 + (i as u64) * 10,
            big_blind: 50 + (i as u64) * 20,
            ante: 50 + (i as u64) * 20,
            duration_minutes: 5,
        })
        .collect();
    ...
    for (name, variant, _table_max, buyin) in configs_mtt {
        let cfg = TournamentConfig {
            ...
            blind_levels: blind_levels.clone(),
            prize_pool_pct: 1.0,
            ...
            allow_addon: false,
            rebuy_max_level: 6,
            guaranteed_prize: if buyin == 0 { 7500 } else { 15000 },
            is_freeroll: buyin == 0,
            rebuy_cost: 1000,
            rebuy_chips: 10000,
            rebuy_max_count: 1,
        ...
        for i in 0..7 {
            register_player(
                &mut state,
                &format!("mtt_{}_{}", name.replace(" ", "_"), i),
                &format!("Player{}", i),
            )
            .unwrap();
        }
        ...
        while state.players_remaining > 1 && level < 26 {
            ...
            let remaining: Vec<String> = state
                .players
                .iter()
                .filter(|(_, e)| e.eliminated_at.is_none())
                .map(|(k, _)| k.clone())
                .collect();
            if remaining.len() <= 1 {
                break;
            ...
            hands += 1;
            if hands % 7 == 0 {
                // avança blind a cada 7 mãos (~5min com 70 mãos/h)
